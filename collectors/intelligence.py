...
TECH_RULES = {
    ...
    "Google Analytics":   ("analytics",      r"google-analytics\.com|googletagmanager\.com/gtag"),
...
NICHE_RULES = [
    ("casino",       r"\bcasino\b|\bgambling\b|\bpoker\b|\bbetting\b|\bsportsbook\b|\bonline.casino\b"),
...
def _classify_b2b_b2c(text: str, niche: str | None) -> str:
    b2b_signals = r"\benterprise\b|\bsolution[s]?\b|\bpartner[s]?\b|\bclient[s]?\b|b2b|\bbusiness\b|\bteam[s]?\b|\bapi\b|\bsaas\b"
    b2c_signals = r"\bshop\b|\bstore\b|\bprice\b|\bbuy\b|\bpurchase\b|\bsale\b|\bcustomer\b|b2c"
    b2b = len(re.findall(b2b_signals, text[:50000], re.I))
    b2c = len(re.findall(b2c_signals, text[:50000], re.I))
    if b2c > b2b:
        return "B2C"
    if b2b > b2c:
        return "B2B"
...
def _extract_phones(text: str, source_url: str) -> list[dict]:
    phones = set(re.findall(r"(?:(?:\+|00)\d{1,3}[\s.-]?)?(?:\(?\d{2,4}\)?[\s.-]?){2,4}\d{3,4}", text))
    return [
        {"kind": "phone", "value": re.sub(r"\s+", " ", p).strip(), "source_url": source_url}
        for p in phones
...
def _extract_seo(soup: BeautifulSoup, url: str) -> dict:
    ...
    og = {x.get("property"): x.get("content") for x in soup.select("meta[property^='og:']")}
    tw = {x.get("name"): x.get("content") for x in soup.select("meta[name^='twitter:']")}
    ...
    structured_data = [s.get_text() for s in soup.select("script[type='application/ld+json']")]

    # dofollow / nofollow
    all_links = soup.select("a[href]")
    dofollow = [a.get("href") for a in all_links if "nofollow" not in (a.get("rel") or [])]
    ...
    if not ("noindex" in robots_meta.lower()): score += 15
...
def _extract_dns(domain: str) -> list[dict]:
    ...
    for rtype in ("A", "AAAA", "MX", "NS", "TXT", "CNAME", "SOA"):
        try:
            ...
            records += [{"record_type": rtype, "record_value": str(r), "error": None} for r in answers]
        except Exception as exc:
            records.append({"record_type": rtype, "record_value": None, "error": type(exc).__name__})
    return records
...
def _marketing_score(seo: dict, social: list, contacts: list, tech: list, pages: dict) -> dict:
    score = 0
    breakdown = {}

    # Website foundation
    ws = 0
    if seo.get("https"): ws += 5  # wait, this is in security; handled separately
    ...
    # Lead gen
    lg = 0
    if any(c["kind"] == "email" for c in contacts): lg += 10
    if any(c["kind"] == "phone" for c in contacts): lg += 5
    if pages.get("has_contact"): lg += 5
    if pages.get("has_pricing"): lg += 5
...
async def full_domain_analysis(domain: str) -> dict:
    ...
    # ── Security ─────────────────────────────────────────
    try:
        result["security"] = _extract_security(dict(resp.headers), str(resp.url))
    except Exception as exc:
        result["errors"].append(f"security: {exc}")

    # ── Technologies ─────────────────────────────────────
    try:
        result["technologies"] = _detect_technologies(html, dict(resp.headers))
    except Exception as exc:
        result["errors"].append(f"technologies: {exc}")
    ...
    try:
        ...
        result["b2b_b2c"] = _classify_b2b_b2c(page_text, result["niche"])
    except Exception as exc:
    ...
    try:
        result["dns"] = _extract_dns(domain)
        # best-effort country from MX/NS records
        mx = [r["record_value"] for r in result["dns"] if r["record_type"] == "MX" and r["record_value"]]
        if mx:
            result["country"] = _guess_country_from_tld(tld or "")
    except Exception as exc:
...
def _guess_country_from_tld(tld: str) -> str | None:
    country_tlds = {
        ".hu": "Hungary", ".uk": "United Kingdom", ".de": "Germany",
        ".fr": "France", ".es": "Spain", ".it": "Italy", ".pl": "Poland",
        ".ru": "Russia", ".br": "Brazil", ".au": "Australia", ".ca": "Canada",
        ".in": "India", ".cn": "China", ".jp": "Japan", ".kr": "South Korea",
        ".nl": "Netherlands", ".be": "Belgium", ".se": "Sweden", ".no": "Norway",
        ".dk": "Denmark", ".fi": "Finland", ".pt": "Portugal", ".ro": "Romania",
        ".cz": "Czech Republic", ".sk": "Slovakia", ".gr": "Greece", ".tr": "Turkey",
        ".ua": "Ukraine", ".za": "South Africa", ".mx": "Mexico", ".ar": "Argentina",
        ".cl": "Chile", ".co": "Colombia", ".nz": "New Zealand", ".sg": "Singapore",
